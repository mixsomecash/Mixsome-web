...
export const getDefiTransactions = async (accountAddress: string, chainId: ChainId) => {
  ...
  const defiTransactions = await Promise.all(
    transfers.result.map(async transfer => {
  ...
  return defiTransactions.filter(transaction => !!transaction)
}
